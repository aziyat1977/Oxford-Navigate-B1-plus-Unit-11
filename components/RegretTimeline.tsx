...
import { RefreshCcw, Rewind, Play, MoveHorizontal } from 'lucide-react';
  ...
  return (
    <div className="w-full my-12 bg-white dark:bg-black p-1 rounded-3xl border-2 border-gray-200 dark:border-gray-800 shadow-2xl overflow-hidden relative group">
      ...
      <div className="absolute top-4 right-4 z-20 flex gap-2">
         <button 
           onClick={toggleSpeed}
           className="bg-black/10 dark:bg-white/10 hover:bg-black/20 dark:hover:bg-white/20 p-3 rounded-full transition-colors backdrop-blur-sm touch-manipulation"
           title={isSlowMo ? "Switch to Normal Speed" : "Switch to Slow Motion"}
         >
            {isSlowMo ? <Play size={18} /> : <Rewind size={18} />}
         </button>
         <button 
      ...
      <div className="relative w-full overflow-x-auto pb-4 md:pb-0 hide-scrollbar">
        <div className="min-w-[700px] md:min-w-full aspect-[2.5/1] min-h-[250px]" key={key}>
        ...
        {/* Scroll Hint */}
        <div className="md:hidden absolute bottom-2 right-4 pointer-events-none text-gray-400 flex items-center gap-1 text-[10px] uppercase font-bold animate-pulse">
