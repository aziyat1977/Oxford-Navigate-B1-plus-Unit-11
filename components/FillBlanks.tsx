...
export const FillBlanks: React.FC = () => {
  ...
  const checkAll = () => {
    ...
    QUESTIONS.forEach(q => {
      ...
      // Allow contractions: shouldn't have == should not have (simplification for this specific exercise focus)
      // The answer key expects standard contracted forms taught in the unit
  ...
  return (
    <div className="bg-gray-100 dark:bg-zinc-900/50 p-8 rounded-2xl border border-gray-200 dark:border-gray-800 my-10">
        ...
        {QUESTIONS.map((q) => {
          ...
          return (
            <div key={q.id} className="bg-white dark:bg-black p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800">
              <div className="flex flex-col md:flex-row md:items-center gap-3 text-lg text-gray-700 dark:text-gray-300">
                <span>{q.prefix}</span>
                <div className="relative flex-1">
                  <input
                    ...
                    className={`w-full bg-gray-50 dark:bg-zinc-900 border-b-2 px-3 py-2 focus:outline-none transition-colors font-mono font-bold
                    ...
                    `}
                  />
                  {result === true && <CheckCircle2 className="absolute right-2 top-2.5 text-green-500" size={20} />}
                  {result === false && <AlertCircle className="absolute right-2 top-2.5 text-red-500" size={20} />}
                </div>
                <span>{q.suffix}</span>
              </div>
              {result === false && (
                <p className="text-red-500 text-xs mt-2 font-mono ml-2">Correction: {q.answer}</p>
              )}
            </div>
          );
        })}
      ...
      <div className="mt-8 flex justify-end">
        <button
          onClick={checkAll}
          className="bg-black dark:bg-white text-white dark:text-black px-6 py-2 rounded-lg font-bold uppercase tracking-wider hover:opacity-80 transition-opacity"
        >
          Check Entries
        </button>
      </div>
